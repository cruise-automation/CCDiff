...
def prepare_hist_in_transformer(pos, yaw, speed, extent, avail, norm_info):
    '''
    INPUTS:
    - pos : (B, T, 2)
    - yaw : (B, T, 1)
...
class AgentHistoryEncoderTransformer(nn.Module): 
    '''
    Causal Transformer encodes past state history.
    '''
    def __init__(self, num_steps,
        ...
        encoder_layer = nn.TransformerEncoderLayer(d_model=out_dim, nhead=num_heads, batch_first=True)
        self.state_embedding = nn.Sequential(
            nn.Linear(self.state_dim, 4*out_dim),
            nn.ReLU(),
            nn.Linear(4*out_dim, out_dim),
            nn.ReLU(), 
        )
        if use_pos_embedding:
            self.pos_embedding = PositionalEncoding(out_dim, max_len=num_steps)
        self.traj_encoder = nn.TransformerEncoder(encoder_layer, num_layers=num_layers)
    ...
    def forward(self, pos, yaw, speed, extent, avail):
        ...
        net_in = prepare_hist_in_transformer(pos, yaw, speed, extent, avail, (self.add_coeffs.to(pos.device), self.div_coeffs.to(pos.device)))
        net_out = self.state_embedding(net_in)
        if self.pos_embedding: 
            net_out += self.pos_embedding(net_in)
        net_out = self.traj_encoder(net_out, mask=mask)[:, :, -1]
        return net_out
    ...
    def scene_forward(self, net_in):
        ...
        - net_out : (B*M, d_k)
        '''
        B, M, T, _ = net_in.shape
        net_out = self.traj_encoder(net_in.reshape(B*M, T, -1))
        return net_out
...
class NeighborHistoryEncoderTransformer(nn.Module):
    '''
    Causal Transformer encodes past state history.
    '''
    def __init__(self, num_steps,
...
from ccdiff.models.base_models import Up, ConvBlock, IdentityBlock
