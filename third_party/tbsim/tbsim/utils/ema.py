...
class EMA(pl.Callback):
    ...
    Implementation detail:
        ...
        - When multi gpu, we broadcast ema weights and the original weights in order to only hold 1 copy in memory.
          ...
          performance.
    """
    def __init__(self, decay: float = 0.995, ema_device: Optional[Union[torch.device, str]] = None, pin_memory=True, every_n_train_steps: int = 10):
        super().__init__()
        self.decay = decay
    ...
    def on_train_batch_end(self, trainer: "pl.Trainer", pl_module: pl.LightningModule, *args, **kwargs) -> None:
        # Update EMA weights
        if self._every_n_train_steps >= 1 and (trainer.global_step % self._every_n_train_steps == 0):
            # print('update ema')
            with torch.no_grad():
