...
def parse_node_centric(batch: dict, overwrite_nan=True):
    ...
    fut_pos, fut_yaw, _, fut_mask = trajdata2posyawspeed(batch["agent_fut"], nan_to_zero=overwrite_nan) # overwrite_nan)
    # print('AGENT HIST: ', batch["agent_hist"].shape, batch["agent_hist"].isnan().any())

    hist_pos, hist_yaw, hist_speed, hist_mask = trajdata2posyawspeed(batch["agent_hist"], nan_to_zero=overwrite_nan) # overwrite_nan)
    curr_speed = hist_speed[..., -1]
    curr_state = batch["curr_agent_state"]
    # print(curr_state.shape, curr_state.heading.shape, curr_state.position.shape)
    curr_state[curr_state.isnan()] = 0.
    # print("CURR state: ",  curr_state.isnan().any(), curr_state.shape)
    # print("CURR state: ",  curr_state.isnan().any(), curr_state.shape)
    # if curr_state.isnan().any().item(): 
    #     print("NAN")
    #     print(batch["agent_hist"])
    #     print(curr_state)
