...
def set_initial_states(env, obs, adjust_plan, device):
    ...
    obs = TensorUtils.to_torch(obs, device,ignore_if_unspecified=True)
...
class RolloutCallback(pl.Callback):
    ...
    def _get_env(self, device):
        if self.env is not None:
            return self.env
        if self._eval_cfg.env == "trajdata":
            env_builder = EnvUnifiedBuilder(eval_config=self._eval_cfg, exp_config=self._exp_cfg, device=device)
        else:
    ...
    def _get_policy(self, pl_module: pl.LightningModule):
        ...
        # CHANGE: Add this to handle trajdata
        elif self._eval_cfg.env == "trajdata":
            rollout_policy = RolloutWrapper(agents_policy=policy)
