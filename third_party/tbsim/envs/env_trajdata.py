...
class EnvUnifiedSimulation(BaseEnv, BatchedEnv):
    ...
    def get_observation(self):
        ...
        # print("RAW OBS: ", agent_obs.keys())

        agent_obs = parse_trajdata_batch(agent_obs, overwrite_nan=True) # TODO: False
        # print("PAR OBS: ", agent_obs.keys())

        # try: 
        #     print("HIST: ", agent_obs["history_positions"].shape)
        # except: 
        #     pass
    ...
    def _step(self, step_actions: RolloutAction, num_steps_to_take):
        ...
        #     try: 
        #         if np.any(np.isnan(action[k])): 
        #             print(k, np.where(np.isnan(action[k]))[0])
        #     except: 
        #         print("11111: ", k, action[k])    
        # print("--------- step ends ----------")
        # input()
        ...
        for action_index in range(num_steps_to_take):
            ...
            for scene in self._current_scenes:
                scene_action = dict()
                # print("SCENE AGENT: ", scene.agents)
                for agent in scene.agents:
                    ...
                    # print("========Action Pos=======")
                    # print('pos: ', np.any(np.isnan(action["positions"][idx, action_index])), np.any(np.isnan(curr_pos)))
                    # print('yaw: ', np.any(np.isnan(action["yaws"][idx, action_index, 0])), np.any(np.isnan(curr_yaw)))
                    # print("===============")
                    ...
                    # print("--------- scene_action begins ----------")
                    # print('SCENE ACTIONS: ', scene_action.keys())
                    # for k in scene_action.keys(): 
                    #     try: 
                    #         if np.any(np.isnan(scene_action[k])): 
                    #             print(k, scene_action[k].shape, np.where(np.isnan(scene_action[k]))[0])
                    #     except: 
                    #         print("scene_action: ", k, scene_action[k])    
                    # print("--------- scene_action ends ----------")
                    # input()
                    idx += 1
