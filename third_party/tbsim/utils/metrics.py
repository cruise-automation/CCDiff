...
def batch_detect_off_road(positions, drivable_region_map):
    ...
    # skip the off-road detection when the agent is not available
    # print(positions.shape)
    idx_nan = torch.where(torch.logical_or(torch.isnan(positions[..., 0]), torch.isnan(positions[..., 1])))
    positions[idx_nan] = 0.

    if positions.dtype in (torch.float32, torch.float64):
        pos_x = torch.round(positions[..., 0])
        pos_y = torch.round(positions[..., 1])
    else:
        pos_x = positions[..., 0]
        pos_y = positions[..., 1]
    positions[..., 0] = torch.clamp(pos_x, 0, w-1)
    positions[..., 1] = torch.clamp(pos_y, 0, h-1)
...
def batch_detect_off_road_boxes(positions, yaws, extents, drivable_region_map):
    ...
    # skip the off-road detection when the agent is not available
    idx_nan = torch.where(torch.logical_or(torch.logical_or(torch.isnan(positions[..., 0]), torch.isnan(yaws[..., 0])), torch.isnan(extents[..., 0])))
...
def batch_detect_off_road_disk(positions, extents, drivable_region_map):
    ...
    """
    B, H, W = drivable_region_map.size()
    # skip the off-road detection if nan occurs
    idx_nan = torch.where(torch.logical_or(torch.isnan(positions[..., 0]), torch.isnan(extents[..., 0])))
    # set dummy value first
    positions[idx_nan] = 0.
    extents[idx_nan] = 0.
    ...
    # plt.figure()
    # plt.scatter(disk_samp[0,:,0].cpu().numpy(), disk_samp[0,:,1].cpu().numpy())
    # plt.scatter(disk_samp[1,:,0].cpu().numpy(), disk_samp[1,:,1].cpu().numpy())
    # plt.show()
