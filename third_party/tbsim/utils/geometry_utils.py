...
def transform_points_np(points, transf_matrix):
    transformed_points = transform_points_tensor(torch.from_numpy(points.astype('float32')), torch.from_numpy(transf_matrix.astype('float32')))
    return transformed_points.cpu().detach().numpy()
...
def transform_points_tensor(
    ...
    def _transform(points: torch.Tensor, transf_matrix: torch.Tensor) -> torch.Tensor:
        ...
        points = points.float()
        transf_matrix = torch.permute(transf_matrix, (0, 2, 1)).float()
        return (
...
def detect_collision(
    ...
    if np.isnan(ego_pos).any() or np.isnan(ego_yaw).any() or np.isnan(ego_extent).any():
        return None
    ...
    for i in range(other_pos.shape[0]):
        if np.isnan(other_pos).any() or np.isnan(other_yaw).any() or np.isnan(other_extent).any():
            continue
