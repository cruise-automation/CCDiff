...
def launch_experiments_ngc(
    ...
    for cfg, cpath in zip(cfgs, cfg_paths):
        ...
        py_cmd = [
            ...
            "cd {}/behavior-generation;".format(ngc_config["workspace_mounting_point"]),
            "pip install -e .; pip install numpy==1.21.4;",
            "cd {}/trajdata;".format(ngc_config["workspace_mounting_point"]),
            "pip install -r trajdata_requirements.txt; pip install -e .;",
            ...
            "pip install einops; ",
            "cd {}/stlcg;".format(ngc_config["workspace_mounting_point"]),
            "pip install -e .;",
            "cd {}/behavior-generation;".format(ngc_config["workspace_mounting_point"]),
            ""
        ]

        if isinstance(ngc_config["dataset_id"], list):
            dataset_cmd = []
            for cur_id, cur_mt_pt in zip(ngc_config["dataset_id"], ngc_config["dataset_mounting_point"]):
                dataset_cmd.append("--datasetid")
                dataset_cmd.append("{}:{}".format(cur_id, cur_mt_pt))
        else:
            dataset_cmd = ["--datasetid", "{}:{}".format(ngc_config["dataset_id"], ngc_config["dataset_mounting_point"])]
        ...
        if append_results_dir:
            py_cmd.extend(["--results_root_dir", os.path.join(ngc_config["eval_output_dir"], cfg.results_dir)])
        py_cmd = " ".join(py_cmd)
        cmd = [
            "ngc",
            "batch",
...
def launch_scene_edit_ngc(
    script_command: list, cfgs: List[Dict], cfg_paths: List[str], ngc_config: dict, name: str, dry_run=False
):
    """
    Launch one or more experiments on NGC
    Args:
    ...
    # TODO update this to just clone the repo instead of running out of workspace
    #       NOTE: still need to get the expt config from the workspace though I guess.
    ...
    py_cmd = [
        ...
        "cd /opt/conda/lib/python3.8/site-packages; rm -rf cv2; "
        "cd {}/tbsim;".format(ngc_config["workspace_mounting_point"]),
        "pip uninstall -y opencv-python; ", ## the Opencv installation is added to resolve a new bug on NGC caused by non-unique cv2 packages
        "pip install opencv-python; ",
        "pip install einops;", # for diffuser
    ]
    if "dataset_zip_mount" in ngc_config and "dataset_zip_tgt" in ngc_config:
        # copy and unzip data on raid for faster loading
        tgt_toks = ngc_config["dataset_zip_tgt"].split("/")
        zip_parent = "/".join(tgt_toks[:-1])
        zip_file_name = tgt_toks[-1]
        data_zip_cmd = [
            "cp {} {};".format(ngc_config["dataset_zip_mount"], ngc_config["dataset_zip_tgt"]),
            "cd {};".format(zip_parent),
            "unzip {};".format(zip_file_name),
            "cd {}/tbsim;".format(ngc_config["workspace_mounting_point"]),
        ]
        py_cmd.extend(data_zip_cmd)
...
def upload_codebase_to_ngc_workspace(ngc_config):
    ...
    ngc_path = os.path.join(
        ngc_config["workspace_mounting_point_local"], "behavior-generation/")
    local_path = Path(tbsim.__path__[0]).parent
    assert os.path.exists(ngc_path), "please mount NGC path first"
    dir_list = ["scripts/", "behavior-generation/"]
    for d in dir_list:
