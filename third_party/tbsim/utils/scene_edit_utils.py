...
def enumerate_keys(input_dict, current_key=""):
    for key, value in input_dict.items():
        new_key = f"{current_key}.{key}" if current_key else key
        if isinstance(value, dict):
            yield from enumerate_keys(value, new_key)
        else:
            yield new_key
...
def guided_rollout(
    ...
    Args:
        ...
        num_steps_intervention: (int) number of the frames that the intervention starts to exert on the selcted agents
    Returns:
    ...
    Partly controllable setting related items: 
        generative model: policy.agents_policy.policy 
        controllable_set: policy.agents_policy.policy.controllable_set
    ...
    if len(guidance_config) > 0:
        ...
        if apply_guidance and eval_class in ['CCDiff', 'Diffuser', 'TrafficSim', 'BC', 'HierarchicalSampleNew']:
    ...
    if len(constraint_config) > 0:
        ...
        if apply_constraints and eval_class in ['CCDiff', 'Diffuser']:
    ...
    matrices = {}
    try: 
        controllable_set_pre = copy.deepcopy(policy.agents_policy.controllable_set)
        policy.agents_policy.set_controllable_set([])
    except: 
        print("fully controllable")
        pass
    while not done:
        try: 
            if counter >= num_steps_intervention: # start to apply guidance?
                policy.agents_policy.set_controllable_set(controllable_set_pre)
        ...
        with timers.timed("network"):
            ...
            # print('action ego info: ',action.to_dict()['agents']['positions'])
            # for k in action.agents_info.keys(): 
            #     matrices.setdefault(k, [])
            #     matrices[k].append(action.agents_info[k])

        # get rollout observation and corresponding actions
        # for k in enumerate_keys(obs_torch['agents']): 
        #     try: 
        #         print(k, obs_torch['agents'][k].shape)
        #     except: 
        #         try: 
        #             print(k, len(obs_torch['agents'][k]))
        #         except:
        #             print(k)
        # for k in enumerate_keys(action.to_dict()): 
        #     print(k)
        # assert 1==2
    ...
    #     matrices[k] = np.stack(matrices[k], axis=0)
    #     print(k, matrices[k].shape)
    # info.update(matrices)
...
def draw_scene_data(ax, scene_name, scene_data, starting_frame, rasterizer, 
    ...
    # mark_agents = []
    # controlled_agents = min(15, len(scene_data["centroid"]))
    # mark_agents = [(i, "*", "red") for i in range(controlled_agents)]
